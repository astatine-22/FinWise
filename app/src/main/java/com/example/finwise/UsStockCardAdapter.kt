...
/**
 * Adapter for US Stock cards - displays prices in USD ($)
 */
class UsStockCardAdapter(
    private var stocks: List<StockItem>,
    private val onClick: (StockItem) -> Unit
) : RecyclerView.Adapter<UsStockCardAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.tvStockSector.text = stock.sector ?: "US Stock"
        
        // Display price in USD ($)
        holder.tvStockPrice.text = dollarFormat.format(stock.price)

        // Set change text with +/- sign
